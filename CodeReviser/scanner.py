...
class BubbleSheetScanner:
    ...
    def _generate_demo_student_info(self):
        ...
        try:
            ...
            # Try to get a random student from the user_info table
            with db.engine.connect() as conn:
                # First check if we have students in our local student table
                count_result = conn.execute(text("SELECT COUNT(*) FROM student"))
                count = count_result.fetchone()[0]
                
                if count > 0:
                    # Get a random student from our local table
                    result = conn.execute(text("SELECT name, student_id FROM student ORDER BY RANDOM() LIMIT 1"))
                    ...
                    # Try to create some students from the checker_db user_info table
                    try:
                        # Insert some students from the user_info table 
                        conn.execute(text("""
                        ...
                        # Now try to get a random student again
                        result = conn.execute(text("SELECT name, student_id FROM student ORDER BY RANDOM() LIMIT 1"))
                        student = result.fetchone()
        ...
        try:
            ...
            for question_num in range(1, question_count + 1):
                ...
                if question_num <= 5:  # Only save first few for debugging
                    ...
                    cv2.imwrite(debug_path, visualization or roi)
